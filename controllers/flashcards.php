...
class ShortcodeRevolutionFlashcards extends ShortcodeRevolutionShortcode {	
	...
	public static function card($atts, $contents = '') {
		...
		$text_color = empty($atts['front_text_color']) ? 'black' : esc_attr($atts['front_text_color']);
		$back_color = empty($atts['back_color']) ? 'white' : esc_attr($atts['back_color']);
		$back_text_color = empty($atts['back_text_color']) ? 'black' : esc_attr($atts['back_text_color']);
		$padding = empty($atts['padding']) ? '10px;' : esc_attr($atts['padding']);
			
		$flashcard = '';	
		$flashcard .= "<div class='srevo-flashcard' style='cursor:pointer; width:" . $width . ";height:" . $height . ";border-radius:" . $border_radius . ";' onclick=\"jQuery(this).flip('toggle');\"><div class='srevo-flashcard-front front' style='padding: ".$padding.";background-color:" . $color . ";color:" . $text_color. ";border-radius:" . $border_radius . ";'>" . $front_side . "</div><div class='srevo-flashcard-back back' style='padding: ".$padding.";background-color:" . $back_color . ";color:" . $back_text_color.";border-radius:" . $border_radius. ";'>" . $back_side . "</div></div>";		
